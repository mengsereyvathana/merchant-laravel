...
{
    public function admin_login(AdminLoginRequest $req): \Illuminate\Http\JsonResponse
    {
        $name = $req->name;
    ...
    public function verify_email_otp(VerifyEmailOtpRequest $req)
    {
        ...
        if (is_numeric($req->emailPhone)) {
            PhoneOtp::where('phone', $req->emailPhone)->where('expired_date', '<=', $current_time)->delete();
            $tbl_email_phone_otp = PhoneOtp::where('phone', $req->emailPhone)->first();
        }
        ...
        if ($tbl_email_phone_otp) {
            ...
            if ($check) {
                ...
                if (is_numeric($req->emailPhone)) {
                    ...
                    PhoneOtp::where('phone', $req->emailPhone)->delete();
                }
                if (filter_var($req->emailPhone, FILTER_VALIDATE_EMAIL)) {

                    User::where('email', $req->emailPhone)->update(['verify' => 1]);
                    PhoneOtp::where('email', $req->emailPhone)->delete();
                }
    ...
    public function resend_otp(Request $req)
    {
        ...
        $resend_otp = new PhoneOtp;
        if (is_numeric($phoneEmail_check)) {
            PhoneOtp::where('phone', $phoneEmail_check)->delete();
            $resend_otp->phone = $phoneEmail_check;
        }
        if (filter_var($phoneEmail_check, FILTER_VALIDATE_EMAIL)) {
            PhoneOtp::where('email', $phoneEmail_check)->delete();
